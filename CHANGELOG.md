...
- **Added real screenshots to the README** (`docs/assets/screenshot-overview.png`, `screenshot-terminal.png`) — the dashboard and the `mg` terminal running `mg log`, both captured from a live, fully TMDB-enriched session rather than mocked up.
- **Fixed an unbounded proxy hang in live sync**: `LB.fetchHTML()` had no request timeout, so a stalled CORS proxy (a TCP connect that never resolves, rather than a clean error) could block the whole fallback chain for the browser's default fetch timeout — well over a minute — before ever reaching a working proxy, most visible as an indefinitely "syncing…" force-sync button. Confirmed live: at the time of this fix, 2 of the 3 configured proxies (`allorigins.win`, `codetabs.com`) were hanging outright and the third (`cors.eu.org`) was rate-limited, so this wasn't a hypothetical. Added an 8-second `AbortController` timeout per proxy attempt; the existing non-fatal fallback behavior (stored history keeps rendering, next poll retries) is unchanged.
