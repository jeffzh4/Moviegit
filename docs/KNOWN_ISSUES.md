...
## Color contrast falls short of WCAG AA in ~70 places, measured not assumed

Wiring an `axe-core` scan into CI (see [ARCHITECTURE.md §8](ARCHITECTURE.md#8-ci-and-verification-gates)) found 71 `color-contrast` violations on first run, almost entirely `--text-tertiary` (`#6e7681` on dark, `#818b98` on light) used for secondary labels — measured at **4.12:1** against `--bg-canvas` in dark mode, against a WCAG AA requirement of 4.5:1 for normal text. This had been an unmeasured assumption in [DESIGN.md §6](DESIGN.md#6-accessibility) until the scan ran; it's disclosed here rather than quietly fixed because the CI job that found it is intentionally non-blocking for now (see [ROADMAP.md](ROADMAP.md) — flipping it to a hard failure before the fix lands would just make the accessibility badge permanently red, which hides the signal instead of surfacing it). The fix itself is narrow and understood: `--text-tertiary` needs to move a few percent lighter (dark mode) / darker (light mode) in both themes; it just hasn't been done yet.
